...
async function openSyncMissingModal() {
  const setCode = document.getElementById('inv-search-input')?.value?.trim().toUpperCase();
  if (!setCode) return;

  const modal = document.getElementById('sync-missing-modal');
  const body  = document.getElementById('sync-missing-body');
  const confirmBtn = document.getElementById('sync-missing-confirm');
  _syncPendingRows = null;
  confirmBtn.style.display = 'none';
  body.innerHTML = '<div class="text-center muted" style="padding:20px">Checking YGOPRODeck…</div>';
  modal.classList.remove('hidden');
  document.body.style.overflow = 'hidden';

  try {
    // Get set_name from existing cards so YGOPRODeck query works
    const existing = [...document.querySelectorAll('#inv-tbody .inv-row')];
    if (!existing.length) throw new Error('Load a set first');

    // Pull set_name from the first visible card's data attribute or from DB
    const params = new URLSearchParams({ card_number: `ilike.${setCode}-%`, select: 'set_name', limit: 1 });
    const snRes  = await fetch(`${SUPABASE_URL}/rest/v1/card_inventory?${params}`, { headers: DB_HEADERS_RETURN });
    const snData = snRes.ok ? await snRes.json() : [];
    const setName = snData[0]?.set_name;
    if (!setName) throw new Error('Could not find set name for ' + setCode);

    // Fetch all cards in this set from YGOPRODeck
    const ygRes  = await fetch(`${YGOPRO}/cardinfo.php?cardset=${encodeURIComponent(setName)}`);
    if (ygRes.status === 400) throw new Error('Set not found on YGOPRODeck');
    if (!ygRes.ok) throw new Error(`YGOPRODeck error ${ygRes.status}`);
    const ygData = (await ygRes.json()).data || [];

    // Build set of existing card_number|rarity keys from DB
    const invParams = new URLSearchParams({ card_number: `ilike.${setCode}-%`, select: 'card_number,rarity,cards(id)', limit: 1000 });
    const invRes    = await fetch(`${SUPABASE_URL}/rest/v1/card_inventory?${invParams}`, { headers: DB_HEADERS_RETURN });
    const invData   = invRes.ok ? await invRes.json() : [];
    const existingInvKeys  = new Set(invData.map(r => `${r.card_number}|${r.rarity}`));
    const existingCardNums = new Map(invData.map(r => [r.card_number, r.cards?.id]));
    ...
    for (const card of ygData) {
      ...
      for (const entry of entries) {
        ...
        if (existingInvKeys.has(invKey)) continue;

        missing.push({ cardNumber, rarity, cardName: card.name });

        if (!existingCardNums.has(cardNumber)) {
          const newId = crypto.randomUUID();
          cardRows.push({ id: newId, card_number: cardNumber, card_name: card.name, set_name: setName, api_id: String(card.id) });
    ...
    if (!missing.length) {
      body.innerHTML = `<div class="text-center" style="padding:20px;color:var(--green)">✓ All YGOPRODeck cards for <strong>${escHtml(setCode)}</strong> are already in your database.</div>`;
      return;
    }

    _syncPendingRows = { cardRows, invRows };
    confirmBtn.textContent = `Add ${invRows.length} missing entr${invRows.length === 1 ? 'y' : 'ies'}`;
    confirmBtn.style.display = '';

    body.innerHTML = `
      <p style="color:var(--txt);margin-bottom:10px">
        Found <strong style="color:var(--yellow)">${missing.length}</strong> card entr${missing.length === 1 ? 'y' : 'ies'} missing from <strong>${escHtml(setCode)}</strong>:
      </p>
      <div style="max-height:240px;overflow-y:auto;border:1px solid var(--b2);border-radius:6px">
        <table style="width:100%;border-collapse:collapse;font-size:0.8rem">
          ${missing.map(m => `
            <tr style="border-bottom:1px solid var(--b1)">
              <td style="padding:5px 8px;color:var(--muted);white-space:nowrap">${escHtml(m.cardNumber)}</td>
              <td style="padding:5px 8px;font-weight:500">${escHtml(m.cardName)}</td>
              <td style="padding:5px 8px;color:var(--muted);white-space:nowrap">${escHtml(m.rarity)}</td>
            </tr>`).join('')}
        </table>
      </div>`;
  } catch (e) {
    body.innerHTML = `<div class="text-center red" style="padding:20px">Error: ${escHtml(e.message)}</div>`;
  }
}

function closeSyncMissingModal() {
  document.getElementById('sync-missing-modal').classList.add('hidden');
  document.body.style.overflow = '';
  _syncPendingRows = null;
}

async function confirmSyncMissing() {
  if (!_syncPendingRows) return;
  const { cardRows, invRows } = _syncPendingRows;
  const btn  = document.getElementById('sync-missing-confirm');
  const body = document.getElementById('sync-missing-body');
  btn.disabled = true;
  btn.textContent = 'Inserting…';

  try {
    // Insert cards in batches of 250
    for (let i = 0; i < cardRows.length; i += 250) {
      await dbUpsert('cards', cardRows.slice(i, i + 250));
    }
    // Insert card_inventory rows in batches of 250
    for (let i = 0; i < invRows.length; i += 250) {
      await dbUpsert('card_inventory', invRows.slice(i, i + 250));
    }
    body.innerHTML = `<div class="text-center" style="padding:20px;color:var(--green)">✓ Added ${invRows.length} missing entr${invRows.length === 1 ? 'y' : 'ies'}. Reloading set…</div>`;
    btn.style.display = 'none';
    setTimeout(() => {
      closeSyncMissingModal();
      const setCode = document.getElementById('inv-search-input')?.value?.trim().toUpperCase();
      if (setCode) loadInventoryCards(setCode, document.getElementById('inv-card-count'));
    }, 1200);
  } catch (e) {
    body.innerHTML = `<div class="text-center red" style="padding:20px">Insert failed: ${escHtml(e.message)}</div>`;
    btn.disabled = false;
    btn.textContent = 'Retry';
  }
}

function showEditionWarnModal() {
  document.getElementById('edition-warn-modal').classList.remove('hidden');
  document.body.style.overflow = 'hidden';
}

function closeEditionWarnModal() {
  document.getElementById('edition-warn-modal').classList.add('hidden');
  document.body.style.overflow = '';
}
...
function initInventory() {
  ...
  const filterInput = document.getElementById('inv-filter-input');
  const filterCount = document.getElementById('inv-filter-count');
  ...
  filterInput.addEventListener('input', () => {
    ...
    let visible = 0;
    rows.forEach(tr => {
      const num = (tr.querySelector('.inv-td-num')?.textContent || '').toUpperCase();
      const name = (tr.querySelector('.inv-td-name')?.textContent || '').toUpperCase();
      const show = !term || num.includes(term) || name.includes(term);
      tr.style.display = show ? '' : 'none';
      if (show) visible++;
    });
    filterCount.textContent = term
      ? `${visible} of ${rows.length} card${rows.length !== 1 ? 's' : ''} shown`
      : '';
  });
...
async function loadInventoryCards(setCode, countEl) {
  ...
  try {
    ...
    table.style.display = '';
    document.getElementById('inv-filter-wrap').style.display = '';
